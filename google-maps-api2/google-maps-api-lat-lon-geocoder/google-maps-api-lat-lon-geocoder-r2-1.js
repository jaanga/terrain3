		...
		menuDetailsGeocoder =
			...
			'<details open >' +
				...
				'<p>' +
					...
					'onchange=addressGeocoder(geocoder,googleMap.map); title="Thank you Google!" > ' +
				'<button onclick=addressGeocoder(geocoder,googleMap.map); > geocode </button></p>' +

				'<p>Latitude : <input id=inpLatitude size=12 value=' + defaults.latitude + ' onclick=this.select(); onchange=onChangeLatLonGeocoder(); ></p>' +
				'<p>Longitude: <input id=inpLongitude size=12 value=' + defaults.longitude + ' onclick=this.select(); onchange=onChangeLatLonGeocoder();  ></p>' +
	...
	function initGoogleGeocoder() {
		...
		googleMap.origin_autocomplete.addListener( 'place_changed', onPlaceChangedGeocoder );

		googleMap.map.addListener( 'idle', onIdleGeocoder );

	}


	function onChangeLatLonGeocoder() {
			...
			geocodeLatLonGeocoder();

	}


	function onPlaceChangedGeocoder( ) {
...
//		menuPlaceMessage.innerHTML = 'Name:' + place.origin + b;
		...
		addressGeocoder( geocoder, googleMap.map );

	}

	function onIdleGeocoder() {
...
// console.log( 'onIdle' );
	...
	function geocodeLatLonGeocoder() {
